...
func main() {
...

	{
		...
		func() Any {
			a := one(vm, NewMap(MapData{
				S43 /* c */ : one(vm, NewMap(MapData{
					S44 /* d */ : one(vm, Func(func(vm *VM, aa *Args) *Args {
						vm.da(aa)
						{
		...
		vm.da(call(vm, Nprint, join(vm, call(vm, find(one(vm, find(Nt, S43 /* c */)), S44 /* d */), join(vm, nil)))))
